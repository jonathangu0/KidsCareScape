...
select *,case when complaint_type not in ('Dirty Conditions','Missed Collection',
'Sanitation Condition','Derelict Vehicles','Graffiti') then 'Other'
else complaint_type end as complaint_type_cal from dsny_new where year = 2022 and status_cal = 'Open'
	) 
	to 'C:\Users\fguo\Documents\work\ad-hoc\20220504dsny\dsny_new_2022_only_open.csv' header csv delimiter ','
*/

/*
select *,case when complaint_type not in ('Dirty Conditions',
'Missed Collection',
...
'Obstruction',
'Storm') then 'Other'
else complaint_type end as complaint_type_cal, extract(quarter from Created_Date) into dsny_new_time from dsny_new 
 */

/*alter table dsny_new_time
rename column date_part to quarter
*/
/* copy (
select year, quarter, complaint_type_cal, count(*), avg(duration), avg(number_of_past_due_days), avg(number_of_days_to_close) from dsny_new_time
group by year, quarter, complaint_type_cal order by 1,2,3)
 to 'C:\Users\fguo\Documents\work\ad-hoc\20220504dsny\dsnySummary_year_quarter_complaint_cal.csv' header csv delimiter ','
 */
/* 
 copy (
select year, month, case when complaint_type not in ('Dirty Conditions','Missed Collection',
'Sanitation Condition','Derelict Vehicles', 'Graffiti') then 'Other'
else complaint_type end as complaint_type_cal, case when number_of_days_to_close <= 7 then 1 else 0 end as efficient


from dsny_new where year >= 2020 
	) 
	to 'C:\Users\fguo\Documents\work\ad-hoc\20220504dsny\dsny_new_2020_efficient.csv' header csv delimiter ','
*/
/*
drop table dsny_new_2020_efficient;
select year, month, case when complaint_type not in ('Dirty Conditions',
'Missed Collection',
'Derelict Vehicles',
'Graffiti',
'snow',
...
'Obstruction'
 ...
 into dsny_new_2020_efficient

from dsny_new where year >= 2020 
*/
/*
copy (
select year, month, complaint_type_cal, sum(efficient), count(*) from dsny_new_2020_efficient
group by year, month, complaint_type_cal order by 1,2,3)
 to 'C:\Users\fguo\Documents\work\ad-hoc\20220504dsny\dsny_new_year_month_comTYpe_efficient.csv' header csv delimiter ','
*/
/*
copy (
select year, month,  sum(efficient), count(*) from dsny_new_2020_efficient
